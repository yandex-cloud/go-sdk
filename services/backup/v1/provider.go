...
func (o *ProviderActivateOperation) Metadata() *backup.ActivateProviderMetadata {
	return o.Operation.Metadata().(*backup.ActivateProviderMetadata)
}
...
// Activate is an operation of Yandex.Cloud Backup Provider service.
// It returns an object which should be used to monitor the operation state.
func (c providerClient) Activate(ctx context.Context, in *backup.ActivateProviderRequest, opts ...grpc.CallOption) (*ProviderActivateOperation, error) {
	connection, err := c.connector.GetConnection(ctx, ProviderActivate, opts...)
	if err != nil {
		return nil, err
	}
	pb, err := backup.NewProviderServiceClient(connection).Activate(ctx, in, opts...)
	if err != nil {
		return nil, err
	}
	op, err := sdkop.NewOperation(pb, &sdkop.Concretization{
		Poll:         c.pollOperation,
		MetadataType: (*backup.ActivateProviderMetadata)(nil),
...
// ListActivated is an operation of Yandex.Cloud Backup Provider service.
func (c providerClient) ListActivated(ctx context.Context, in *backup.ListActivatedProvidersRequest, opts ...grpc.CallOption) (*backup.ListActivatedProvidersResponse, error) {
	connection, err := c.connector.GetConnection(ctx, ProviderListActivated, opts...)
	if err != nil {
		return nil, err
	}
	return backup.NewProviderServiceClient(connection).ListActivated(ctx, in, opts...)
}
...
var (
	ProviderActivate        = protoreflect.FullName("yandex.cloud.backup.v1.ProviderService.Activate")
	ProviderListActivated   = protoreflect.FullName("yandex.cloud.backup.v1.ProviderService.ListActivated")
