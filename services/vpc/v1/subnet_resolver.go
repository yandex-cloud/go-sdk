...
func SubnetResolver(name string, client SubnetClient, opts ...sdkresolvers.ResolveOption) sdkresolvers.Resolver {
	return &subnetResolver{
		client:           client,
		BaseNameResolver: sdkresolvers.NewBaseNameResolver(name, "Subnet", opts...),
	}
}

func (r *subnetResolver) Run(ctx context.Context) error {
	err := r.EnsureFolderID()
	if err != nil {
		return err
	}

	resp := r.client.Iterator(ctx, &vpc.ListSubnetsRequest{
		FolderId: r.FolderID(),
