// Code generated by sdkgen. DO NOT EDIT.

// nolint
package cic

import (
	"context"

	"google.golang.org/grpc"

	cic "github.com/yandex-cloud/go-genproto/yandex/cloud/cic/v1"
)

//revive:disable

// PartnerServiceClient is a cic.PartnerServiceClient with
// lazy GRPC connection initialization.
type PartnerServiceClient struct {
	getConn func(ctx context.Context) (*grpc.ClientConn, error)
}

// Get implements cic.PartnerServiceClient
func (c *PartnerServiceClient) Get(ctx context.Context, in *cic.GetPartnerRequest, opts ...grpc.CallOption) (*cic.Partner, error) {
	conn, err := c.getConn(ctx)
	if err != nil {
		return nil, err
	}
	return cic.NewPartnerServiceClient(conn).Get(ctx, in, opts...)
}

// List implements cic.PartnerServiceClient
func (c *PartnerServiceClient) List(ctx context.Context, in *cic.ListPartnersRequest, opts ...grpc.CallOption) (*cic.ListPartnersResponse, error) {
	conn, err := c.getConn(ctx)
	if err != nil {
		return nil, err
	}
	return cic.NewPartnerServiceClient(conn).List(ctx, in, opts...)
}

type PartnerIterator struct {
	ctx  context.Context
	opts []grpc.CallOption

	err           error
	started       bool
	requestedSize int64
	pageSize      int64

	client  *PartnerServiceClient
	request *cic.ListPartnersRequest

	items []*cic.Partner
}

func (c *PartnerServiceClient) PartnerIterator(ctx context.Context, req *cic.ListPartnersRequest, opts ...grpc.CallOption) *PartnerIterator {
	var pageSize int64
	const defaultPageSize = 1000
	pageSize = req.PageSize
	if pageSize == 0 {
		pageSize = defaultPageSize
	}
	return &PartnerIterator{
		ctx:      ctx,
		opts:     opts,
		client:   c,
		request:  req,
		pageSize: pageSize,
	}
}

func (it *PartnerIterator) Next() bool {
	if it.err != nil {
		return false
	}
	if len(it.items) > 1 {
		it.items[0] = nil
		it.items = it.items[1:]
		return true
	}
	it.items = nil // consume last item, if any

	if it.started && it.request.PageToken == "" {
		return false
	}
	it.started = true

	if it.requestedSize == 0 || it.requestedSize > it.pageSize {
		it.request.PageSize = it.pageSize
	} else {
		it.request.PageSize = it.requestedSize
	}

	response, err := it.client.List(it.ctx, it.request, it.opts...)
	it.err = err
	if err != nil {
		return false
	}

	it.items = response.Partners
	it.request.PageToken = response.NextPageToken
	return len(it.items) > 0
}

func (it *PartnerIterator) Take(size int64) ([]*cic.Partner, error) {
	if it.err != nil {
		return nil, it.err
	}

	if size == 0 {
		size = 1 << 32 // something insanely large
	}
	it.requestedSize = size
	defer func() {
		// reset iterator for future calls.
		it.requestedSize = 0
	}()

	var result []*cic.Partner

	for it.requestedSize > 0 && it.Next() {
		it.requestedSize--
		result = append(result, it.Value())
	}

	if it.err != nil {
		return nil, it.err
	}

	return result, nil
}

func (it *PartnerIterator) TakeAll() ([]*cic.Partner, error) {
	return it.Take(0)
}

func (it *PartnerIterator) Value() *cic.Partner {
	if len(it.items) == 0 {
		panic("calling Value on empty iterator")
	}
	return it.items[0]
}

func (it *PartnerIterator) Error() error {
	return it.err
}
